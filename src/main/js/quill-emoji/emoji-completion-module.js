...
class EmojiCompletionModule extends Module {
  ...
  updateCompletions() {
    ...
    LOG && console.log('  updateCompletions', 'event', `"${window.event}"`, 'colIndex', this.colIndex, 'index', index,
      this.quill.getText().split(), this.quill.getText().length)
  ...
  handleDefaultEditorKeys() {
    ...
    // Note: in Chrome for regular keys "event" is undefined (in contrast to Firefox) but for Enter/Tab is defined
    if (event?.key === 'Enter') {
