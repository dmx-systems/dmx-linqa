  ...
  methods: {
    ...
    onPanStart (e) {
      if (e.inputEvent.touches?.length === 2) {
        ...
        // Allow canvas-pan even if interaction did not start directly on canvas, but only if
        //   1. interaction started on an item (but not on its discussion button) AND
        //   2. item is NOT modifiable/draggable (that is user is not an author or we're in presentation mode)
        const target = e.inputEvent.target
        if (target !== this.$refs.canvas) {
          const item = target.closest('.lq-canvas-item')
          const button = target.closest('.discussion-button')
          if ((!this.isAuthor || this.presentationMode) && item && !button) {
            // Allow canvas-pan (don't call e.stopDrag())
            // Note: allow canvas-pan only if interaction did not start at discussion button, otherwise the discussion
            // button no longer fires "click" on mobile when in presentation mode, I don't know yet why exactly this is
            LOG && console.log('onPanStart() -> STARTING CANVAS PAN (clicked item is read-only)')
          } else {
